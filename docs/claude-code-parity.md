...
Shipped as `.nib/commands/<name>.md`. Loader scans project + global dirs, merges by name (project shadows global). Frontmatter: `description`, `argument-hint` (Claude Code also has `allowed-tools`, `model`, `disable-model-invocation` — not yet implemented). Body serves as prompt template; `$ARGUMENTS` replaced with trailing args via `expandCommand()` in App.tsx's handleSlashCommand. Surfaces in Tab completion and `/help`. No TOFU gate needed (pure prompt injection). ([src/commands/index.ts](../src/commands/index.ts))
